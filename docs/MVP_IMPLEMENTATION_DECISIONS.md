...
- Energy is tracked as produced versus demanded per player. Each powered consumer has an `EnergyBuffer` with capacity `PowerDemand × 100`. The grid fills buffers emptiest-first each tick: sort by fill ratio ascending (`a.Buffer * b.Capacity` vs `b.Buffer * a.Capacity`, integer cross-multiply) then entity id, and distribute `PowerProduced` one energy unit at a time. Assemblers/factories/smelters drain `PowerDemand` each work tick and pause progress when empty. Mines and labs advance `WorkTicks*` without per-tick drain and call energy consume on cycle complete (see mine/lab bullets); failed consume clears the cycle so the next tick can restart when powered.
