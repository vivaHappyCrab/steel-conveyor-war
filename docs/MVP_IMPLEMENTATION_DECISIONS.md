...
- **Partial data-driven behavior (issue #82 slice):** `entities.json` `lossCondition: "Defeat"` drives victory/defeat evaluation via `EntityCatalog.GetDefeatLossKinds()` → `GameSimulation.CheckVictory`. Empty entity catalog (unit-test default) keeps the MVP commander-survival fallback. Tile JSON `walkable` is authoritative for ground pathing, building placement, and flying stop tiles (`TerrainType.Mountain` is not walkable). `resource` and entity `buildsStructures` remain registry/metadata and do **not** yet replace enum-driven extraction or production. Do **not** claim fully data-driven entities/tiles.
