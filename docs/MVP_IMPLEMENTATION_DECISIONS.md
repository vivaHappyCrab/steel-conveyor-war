...
- Composition roots: `SteelConveyorWar.Client` opens an SFML window; `SteelConveyorWar.Headless` is a Core-only host that loads `config/`, calls `CreateNewGame`, applies a stub command/AI step, then `AdvanceTick` (usable from CI without a display). Full local AI opponent (issue #60) and network transport remain follow-ups — see [`docs/engineering/ISSUE_60_LOCAL_AI_OPPONENT.md`](engineering/ISSUE_60_LOCAL_AI_OPPONENT.md). Fair bots use the command queue + `IPlayerView` (`Fair` / `CaptureFrame`), not immediate `Try*` / `World`. Until content freeze the issue stays `ai-blocked`; non-default seeds must no-op.
...
- **Partial data-driven behavior (issue #82 slice):** `entities.json` `lossCondition: "Defeat"` drives victory/defeat evaluation via `EntityCatalog.GetDefeatLossKinds()` → `GameSimulation.CheckVictory`. Empty entity catalog (unit-test default) keeps the MVP commander-survival fallback. Tile JSON `walkable` is catalog metadata (hashed in the content manifest) and currently matches `TerrainType.IsWalkable()` — Mountain is the only non-walkable terrain. Runtime ground pathing, building placement, and flying stop tiles still use the enum helper, not a live lookup of the tile catalog. `resource` and entity `buildsStructures` remain registry/metadata and do **not** yet replace enum-driven extraction or production. Do **not** claim fully data-driven entities/tiles.
...
- Building footprints are explicit in `config/gameplay-tables.json`: extractors/smelter/assembler/lab/hub/Refinery `2x2`; Bastion and military factories `3x3`; CoalPlant `2x3` (unrotated AABB); belts/inserters/walls/turrets/solar `1x1`. Units and the commander use `collisionRadius`, not footprints. Missing building footprints fail cross-catalog validation on non-empty tables.
...
- Research is data-driven through `ResearchCatalog` / `config/research.json`. Runtime ships **only** profile `mvp-b` (serial primary track, three fixed T1/T2 mandatories, optional pools, paired doctrines). Profiles A/C/hybrid remain design history in `docs/MVP_RESEARCH_TREE_VARIANTS.md`. The engine stays composable (no `switch(profile)`); unknown profile ids fail closed.
...
- **Observation API (bots / future net clients):** `GameSimulation.CreatePlayerView(playerId, mode)` returns `IPlayerView` with **immutable tick-stamped snapshots** (R04/R19/M10) — not live mutable `WorldEntity` handles on the fair path. Nested collections are deep-frozen (cast-mutation throws). Prefer `CaptureFrame()` for atomic map+entities+economy decisions: it includes a tick-frozen `FogBoardView` (dense visibility + known terrain for Explored/Visible) alongside entities, own economy/research, tech signatures, command vocabulary, and H06-safe combat events.
  - **Fair** (default): FoW-limited read model — `TryGetTerrain` hides `Unknown` tiles; visible entities/economy/research/combat events are copied snapshots. Prefer this for product AI. Issue #60 v1 may add **authored** attack coordinates (Player 1 start from the map roster) as documented scripted knowledge — that is not Cheat mode.
...
- `GameSimulation.CreateNewGame` builds mirrored starting terrain in memory via `CreateStartingTerrain` (seed + map roster + gameplay tables) and stores `RandomSeed` on the simulation. Player `TeamId` and presentation `Color` come from `MapSettings`. `CreateStartingEntities` iterates the roster and places БМК/Bastion/Hub from resolved start tiles; solar remains algorithmic (adjacent grass / mirrored fairness).
- Terrain generation uses a local `System.Random` seeded with `RandomSeed` to jitter patch centers/radii on the left half, then mirrors **all non-grass tiles** (resources and mountains) to the right for PvP fairness. Grass remains the default fill. The RNG is not kept for later ticks.
- Starting Fe/Cu patches sit near each base; coal/oil patches sit farther toward the half-map center. Ore fill uses **Chebyshev** distance so each patch AABB is at least **4×4** (radius ≥ 2 → 5×5). `tiles.json` lists grass, iron, copper, coal, oil, and mountain.
- Mountain chunks generate after resources: **≤ 5%** of map tiles, Chebyshev **≥ 10** from any resource tile, each axis-aligned chunk **min 2×5** (or 5×2), chunks may share edges but must not overlap tiles or start commander/bastion/hub/solar-candidate tiles. Chunks that would cut left-half routes from the Blue commander to coal, oil, or the midline are rejected. Ground units and buildings cannot enter mountains; flying units may transit them but only stop on walkable tiles.
- `entityStats.movementType` is `Ground` or `Flying`. MVP Flying is **Scout only**. Ground pathing is blocked by non-walkable terrain and buildings. Flying paths through buildings, mountains, and units; the destination/idle tile must be walkable. This is not T3 aviation.
...
- Local AI opponent (issue #60): Headless still has a fixed stub; Client has no CPU flag. Implementation plan (module, queue+fair view, seed lock / no-op, blueprint+mirror) is in [`docs/engineering/ISSUE_60_LOCAL_AI_OPPONENT.md`](engineering/ISSUE_60_LOCAL_AI_OPPONENT.md). Do not start `task-to-pr` until the issue is `ai-ready`.
